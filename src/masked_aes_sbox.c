...
void masked_aes_sbox_8(uint16_t state[8][8])
{
    ...
    masked_and_8(tmp5, state[5], tmp7, tmp8, tmp15, tmp16); // t2 = y12 x y15; t3 = y3 x y6
    masked_xor_8(tmp16, tmp15, tmp17); // t4 = t3 + t2
    masked_and_8(tmp1, state[2], state[3], state[7], tmp18, tmp19); // t7 = y13 x y16; t5 = y4 x U7
    masked_xor_8(tmp19, tmp15, tmp19); // t6 = t5 + t2
    masked_and_8(tmp6, tmp11, state[6], tmp4, tmp20, tmp21); // t10 = y2 x y7; t8 = y5 x y1
    masked_xor_8(tmp21, tmp18, tmp21); // t9 = t8 + t7
    masked_xor_8(tmp20, tmp18, tmp18); // t11 = t10 + t7
    masked_and_8(tmp2, tmp10, tmp0, tmp12, tmp20, tmp22); // t12 = y9 x y11; t13 = y14 x y17
    ...
    masked_and_8(tmp22, tmp14, tmp3, tmp9, tmp21, state[1]); // t26 = t21 x t23; t15 = y8 x y10
    ...
    masked_and_8(tmp21, tmp22, tmp19, tmp20, tmp21, tmp22); // t32 = t31 x t30; t28 = t25 x t27
    ...
    masked_and_8(tmp21, state[7], tmp18, tmp22, state[7], tmp18); // z2 = t33 x U7; t36 = t24 x t35
    ...
    masked_and_8(tmp20, state[5], tmp13, tmp18, tmp22, tmp18); // z0 = t44 x y15; t39 = t29 x t38
    ...
    masked_and_8(tmp14, tmp8, state[4], state[2], tmp8, state[2]); // z1 = t37 x y6; z3 = t43 x y16
    masked_and_8(tmp18, tmp4, tmp13, tmp11, tmp23, tmp11); // z4 = t40 x y1; z5 = t29 x y7
    masked_and_8(state[1], tmp10, state[5], tmp12, tmp10, tmp12); // z6 = t42 x y11; z7 = t45 x y17
    masked_and_8(tmp19, tmp9, tmp20, tmp5, tmp9, tmp5); // z8 = t41 x y10; z9 = t44 x y12
    masked_and_8(tmp14, tmp7, tmp21, state[3], tmp7, tmp14); // z10 = t37 x y3; z11 = t33 x y4
    masked_and_8(state[4], tmp1, tmp18, state[6], tmp1, tmp16); // z12 = t43 x y13; z13 = t40 x y5
    masked_and_8(tmp13, tmp6, state[1], tmp2, tmp6, tmp2); // z14 = t29 x y2; z15 = t42 x y9
    masked_and_8(state[5], tmp0, tmp19, tmp3, tmp0, tmp3); // z16 = t45 x y14; z17 = t41 x y8
